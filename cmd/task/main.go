...
import (
	...
	"github.com/richard-on/task-service/pkg/server"
	"os"
	"runtime"
	"strconv"
	"time"
...
// @title         Task Coordination Service API
// @version       1.0
...
func main() {
	...
	if !fiber.IsChild() {
		...
		log.Infof("task-service - version: %v; build: %v; FiberPrefork: %v; MaxCPU: %v", version, build, config.FiberPrefork, config.MaxCPU)
	}
